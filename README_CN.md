...
一个点电荷 q 以恒定速度 v = v*z_hat 在真空中运动。

## 1. 电磁场
利用 Liénard-Wiechert 势，推导 E(r, t) 和 B(r, t)。
...
1. **Clarifier（澄清器）** 解析问题，提取关键概念，将问题分解为子任务。
2. **Solver（求解器）** 为每个子任务生成详细解答。
3. **Critic（评估器）** 评估解答并做出决定：`complete`（完成）| `to_revise`（修订）| `to_redraft`（重做）。
4. 若未完成，求解器根据评估反馈修订或重做解答。
5. **Summarizer（总结器）** 生成所有已完成子任务的最终 Markdown 总结。
